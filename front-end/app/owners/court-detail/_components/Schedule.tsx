...
function Schedule({ courtID }: { courtID: number }) {
  ...
  const [timeFrame, setTimeFrame] = useState(
    new Date() > new Date(new Date().setHours(12, 0, 0, 0)) ? "afternoon" : "morning",
  )
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      try {
        ...
        const response = await subCourtScheduleApi.getScheduleByCourtId(selectedSubCourt, formattedDate)
    ...
    <div className="w-full">
            ...
            {subCourts.map((subCourt) => (
              ...
              >
                <span className="text-lg font-semibold">
                  {subCourt.subName} - {subCourt.type === "DOUBLE" ? "Đôi" : "Đơn"}
                </span>
              </button>
            ))}
            {subCourts.length === 0 && <span className="text-lg text-gray-500 py-3">Không có dữ liệu sân phụ</span>}
          </div>
        </div>
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center justify-between px-4 py-2 bg-white border border-gray-200 rounded-lg shadow-lg h-14 w-1/3 my-4">
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className="text-lg font-semibold text-gray-900">
                {new Date(date.getTime() + 6 * 24 * 60 * 60 * 1000).toLocaleDateString("vi-VN", {
                  day: "2-digit",
                  month: "2-digit",
                  year: "numeric",
                })}
      ...
      <div className="w-full mt-2 bg-white rounded-lg shadow-lg py-3">
        {sevenDays.map((day, index) => {
          ...
          const daySchedules =
            selectedSubCourt !== null && schedule[selectedSubCourt] ? schedule[selectedSubCourt][keyDate] || [] : []
          ...
          return (
            <div key={index} className="flex items-center justify-between w-full py-3">
              <div className="rounded-lg ml-2 w-1/12 justify-evenly items-center flex">
                <div
                  className={`text-center py-3 w-1/2 rounded-lg border border-primary-500 ${day.toDateString() === new Date().toDateString() ? "bg-primary-500" : ""
                    }`}
                >
                  <span
                ...
                {filteredSchedules.length > 0 ? (
                  filteredSchedules.map((item, idx) => (
                    ...
                    >
                      ...
                      <span className="block text-lg text-gray-500 text-center">{item.price / 1000}k/h</span>
                      {JSON.parse(localStorage.getItem("user") || "{}")?.role === "ADMIN" && (
                        <div
